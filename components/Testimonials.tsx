...
const Testimonials: React.FC = () => {
  return (
    <section id="testimonials" className="py-24 bg-brand-surface">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <span className="text-brand-orange font-bold uppercase tracking-widest text-sm mb-2 block">Testimoni</span>
          <h2 className="text-4xl md:text-5xl font-bold text-brand-dark mb-4 tracking-tight">Apa Kata Penumpang?</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {reviews.map((review, idx) => (
            <div key={idx} className="bg-white p-8 rounded-[2rem] shadow-apple hover:shadow-xl transition-shadow border border-slate-100 relative group">
              ...
              <p className="text-slate-600 mb-8 leading-relaxed font-medium">"{review.text}"</p>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-medium uppercase tracking-wider">{review.from} → {review.to}</p>
                </div>
              </div>
            </div>
          ))}
